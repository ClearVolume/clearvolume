...
public class OpenCLDenoise extends OpenCLProcessor<Boolean> implements
	...
	public void initBuffersBF(final long Nx, final long Ny, final long Nz) {
	...
	public void initBuffersNLM(final long Nx, final long Ny, final long Nz) {
		final long lLength = Nx * Ny * Nz;
		final OpenCLDevice lDev = getDevice();
	...
	private void process_bilateral(int pRenderLayerIndex, long pWidthInVoxels,
		...
		try {
			...
			mKernelBilateral.setArgs(getVolumeBuffers()[pRenderLayerIndex],
					mBufScratch, mBlockSize, mBF_SigmaSpace, mSigma);
	...
	private void process_nlm_fast(int pRenderLayerIndex, long pWidthInVoxels,
		...
		try {
			...
			getDevice().run(mKernelNLM_startup, (int) pWidthInVoxels,
					(int) pHeightInVoxels, (int) pDepthInVoxels);
				...
				for (int dy = -mNLM_BlockSearchSize; dy < mNLM_BlockSearchSize + 1; dy++)
					for (int dz = -mNLM_BlockSearchSize; dz < mNLM_BlockSearchSize + 1; dz++) {
						...
						mKernelNLM_dist.setArgs(
								getVolumeBuffers()[pRenderLayerIndex],
								mBuf_NLM_dist, dx, dy, dz, (2 * mBlockSize + 1)
										* (2 * mBlockSize + 1));
						...
						mKernelNLM_comp_plus.setArgs(
								getVolumeBuffers()[pRenderLayerIndex],
								mBufScratch, mBuf_NLM_acc, mBuf_NLM_weight, dx,
								dy, dz, mSigma);
	...
	private void process_nlm(int pRenderLayerIndex, long pWidthInVoxels,
		...
		try {
			...
			mKernelNLM_Simple
					.setArgs(getVolumeBuffers()[pRenderLayerIndex],
							mBufScratch, mBlockSize, mNLM_BlockSearchSize,
							(float) (.1));
