...
ClearVolumeRendererBase implements ClearGLEventListener
{
	...
	private OculusRiftController ovr = null;
	...
	private void displayInternalScenegraph(final GLAutoDrawable pDrawable) {
		boolean lTryLock = true;

		for (Node n : this.scene.getInitList()) {
			n.init();
		}
		this.scene.getInitList().clear();
		...
		if (lTryLock)
			try {
				...
				boolean lOverlay2DChanged = isOverlay2DChanged();
				boolean lOverlay3DChanged = isOverlay3DChanged();


				GL lGL = pDrawable.getGL();
				...
				for (int eye = 0; eye < eyeCount; eye++) {
					...
					for (Node n : rootNode.getChildren()) {
						...
						if (n.getNodeType().compareTo("VolumeNode") == 0) {
							// set the volume's matrices according to current camera settings
							n.setModel(GLMatrix.getIdentity());
							n.getModel().translate(n.getPosition().x(), n.getPosition().y(), n.getPosition().z());
						}

						GLMatrix camrot = GLMatrix.fromQuaternion(cam.getRotation());
						GLMatrix mv = cam.getView().clone();
						if (System.getProperty("ClearVolume.EnableVR") != null) {
							mv.translate(eyeShift[3 * eye + 0] * 5,
											eyeShift[3 * eye + 1] * 5,
											eyeShift[3 * eye + 2] * 5);

							if (cam.getClass() == DetachedHeadCamera.class) {
								mv.translate(
												getTranslationRotationControllers().get(0).getTranslationVector()[0],
												getTranslationRotationControllers().get(0).getTranslationVector()[1],
												getTranslationRotationControllers().get(0).getTranslationVector()[2]
								);

							}

							mv.mult(getTranslationRotationControllers().get(0).getQuaternion());

						}
						mv.mult(camrot);

						mv.translate(getTranslationX(),
										getTranslationY(),
										getTranslationZ());

						mv.mult(getQuaternion());

						mv.mult(n.getModel());

						GLMatrix proj = cam.getProjection().clone();

						GLMatrix mvp = proj.clone();
						mvp.mult(mv);

						if (n.getNodeType().compareTo("VolumeNode") == 0) {
							n.setModelView(mv.clone());
							n.setProjection(proj.clone());//cam.projection!!.clone()
						}

						if (n.getNodeType() == "Camera") {
							continue;
						}
						...
						if (n.getProgram() != null) {
							n.getProgram().use(lGL);
							n.getProgram().getUniform("ModelViewMatrix").setFloatMatrix(mv, false);
							n.getProgram().getUniform("ProjectionMatrix").setFloatMatrix(cam.getProjection(), false);
							n.getProgram().getUniform("MVP").setFloatMatrix(mvp, false);
							n.getProgram().getUniform("offset").setFloatVector3(n.getPosition().toFloatBuffer());

							n.getProgram().getUniform("Light.Ld").setFloatVector3(1.0f, 1.0f, 0.8f);
							n.getProgram().getUniform("Light.Position").setFloatVector3(-5.0f, 5.0f, 5.0f);
							n.getProgram().getUniform("Light.La").setFloatVector3(0.4f, 0.4f, 0.4f);
							n.getProgram().getUniform("Light.Ls").setFloatVector3(0.0f, 0.0f, 0.0f);
							n.getProgram().getUniform("Material.Shinyness").setFloat(0.5f);
							n.getProgram().getUniform("Material.Ka").setFloatVector3(n.getPosition().toFloatBuffer());
							n.getProgram().getUniform("Material.Kd").setFloatVector3(1.0f, 0.5f, 0.0f);
							n.getProgram().getUniform("Material.Ks").setFloatVector3(1.0f, 0.0f, 0.8f);
						} else {
							System.err.println("Could not find program for " + n.toString());
						}

						if (n.getNodeType().compareTo("VolumeNode") == 0) {
							mLayerTextures[((VolumeNode) n).getLayer()].bind(((VolumeNode) n).getLayer());
						}
						System.out.println("Drawing " + n.hashCode() + ", eye=" + eye);
						n.draw();
					}
